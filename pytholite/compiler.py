...
from pytholite.expr import *
from pytholite import transel
from pytholite.io import gen_io
...
def _process_function_args(symdict, function_def, args, kwargs):
    ...
    ad_positional = _make_function_args_dict(undefined, symdict,
                                             defargs.args, defargs.defaults)
    vararg_name = defargs.vararg
    kwarg_name = defargs.kwarg
    ad_kwonly = _make_function_args_dict(undefined, symdict,
                                         defargs.kwonlyargs,
                                         defargs.kw_defaults)
    ...
    if vararg and vararg_name is None:
        raise TypeError("Function takes {} positional arguments "
                        "but {} were given"
                        .format(len(ad_positional),
...
class _Compiler:
    ...
    def visit_assign_special(self, sa, node, statements):
        ...
        if callee == transel.Register:
            ...
            return self.visit_io_pattern(sa, node.targets, callee,
                                         value.args, value.keywords,
                                         statements)
    ...
    def visit_io_pattern(self, sa, targets, model, args, keywords, statements):
        ...
        states, exit_states = gen_io(self, modelname, model,
                                     args, keywords, from_model)
    ...
    def visit_if(self, sa, node):
        ...
        sa.assemble([test_state] + states_t + states_f, exit_states)
    ...
    def visit_for(self, sa, node):
        ...
        if target in self.symdict:
            raise NotImplementedError(
                "For loop target must use an available name")
    ...
    def visit_expr_statement(self, sa, node):
        if isinstance(node.value, ast.Yield):
            ...
            if (not isinstance(yvalue, ast.Call)
                    or not isinstance(yvalue.func, ast.Name)):
                raise NotImplementedError("Unrecognized I/O pattern")
            callee = self.symdict[yvalue.func.id]
            states, exit_states = gen_io(self, None, callee,
                                         yvalue.args, yvalue.keywords, [])
            sa.assemble(states, exit_states)
        else:
            raise NotImplementedError
...
class Pytholite(UnifiedIOObject):
    ...
    def _compile(self):
        ...
        states = _Compiler(self, symdict, registers).visit_top(
